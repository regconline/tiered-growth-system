...
export default function Navbar() {
  ...
  const linkActive   = `${linkBase} bg-primary text-white`;
  const linkInactive = `${linkBase} text-white/85 hover:text-white hover:bg-white/10`;
    ...
    >
      <div className="container max-w-6xl mx-auto px-4">
        <div className="flex items-center justify-between h-[68px]">
          ...
          <button
            className="md:hidden inline-flex items-center justify-center p-2.5 rounded-lg text-white bg-white/10 hover:bg-white/20 border border-white/20 transition-colors"
        ...
        {/* Mobile menu */}
        <div className={`md:hidden overflow-hidden transition-all duration-300 ease-out ${mobileOpen ? "max-h-[85vh]" : "max-h-0"}`}>
          <div className="pb-4 pt-2 border-t border-white/15 overflow-y-auto max-h-[calc(85vh-4rem)]">
            <Link href="/"       className="block px-4 py-3 text-sm font-semibold text-white hover:bg-white/10 rounded-lg mx-1" onClick={() => setMobileOpen(false)}>Home</Link>
            <Link href="/about/" className="block px-4 py-3 text-sm font-semibold text-white hover:bg-white/10 rounded-lg mx-1" onClick={() => setMobileOpen(false)}>About</Link>

            <button className="flex items-center justify-between w-full px-4 py-3 text-sm font-semibold text-white hover:bg-white/10 rounded-lg mx-1"
            ...
            {mobileSubmenu === "services" && (
              <div className="bg-white/5 rounded-xl mx-2 mb-1 py-1 pl-3">
                <Link href="/services/" className="block px-4 py-2 text-sm font-semibold text-white" onClick={() => setMobileOpen(false)}>All Services</Link>
                {serviceDetails.map((s) => (
                  <Link key={s.slug} href={`/services/${s.slug}/`} className="block px-4 py-2 text-sm text-white/85 hover:text-white" onClick={() => setMobileOpen(false)}>{s.icon} {s.title}</Link>
                ))}
              </div>
            )}
            ...
            {mobileSubmenu === "domains" && (
              <div className="bg-white/5 rounded-xl mx-2 mb-1 py-1 pl-3">
                <Link href="/domains/" className="block px-4 py-2 text-sm font-semibold text-white" onClick={() => setMobileOpen(false)}>All Industries</Link>
                {domains.map((d) => (
                  <Link key={d.slug} href={`/domains/${d.slug}/`} className="block px-4 py-2 text-sm text-white/85 hover:text-white" onClick={() => setMobileOpen(false)}>{d.emoji} {d.shortName}</Link>
                ))}
              </div>
            )}

            <Link href="/locations/" className="block px-4 py-3 text-sm font-semibold text-white hover:bg-white/10 rounded-lg mx-1" onClick={() => setMobileOpen(false)}>Locations</Link>
            <Link href="/blog/"      className="block px-4 py-3 text-sm font-semibold text-white hover:bg-white/10 rounded-lg mx-1" onClick={() => setMobileOpen(false)}>Blog</Link>
            <Link href="/contact/"   className="block px-4 py-3 text-sm font-semibold text-white hover:bg-white/10 rounded-lg mx-1" onClick={() => setMobileOpen(false)}>Contact</Link>
